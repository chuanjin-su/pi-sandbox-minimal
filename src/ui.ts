...
/**
 * Interactive permission prompt with the same options and semantics as
 * pi-sandbox: allow for session only, abort, or allow persistently for this
 * project / all projects. Allow rules can be edited with Tab; persistent
 * grants require an extra Enter to confirm. Optional timeout auto-aborts
 * (never grants).
 */
...
export function formatSandboxConfiguration(
	...
	return [
		"Sandbox Status",
		`  Enabled (this session): ${enabled ? "yes" : "no"}`,
		`  OS sandbox active:      ${enabled && initialized ? "yes" : "no"}`,
		`  Platform:               ${platform}${supported ? "" : " (sandboxing unsupported)"}`,
		`  ! command sandboxing:   ${config.sandboxUserShell === false ? "off" : "on"}`,
		`  Prompt timeout:         ${config.permissionPromptTimeoutSeconds ?? DEFAULT_PERMISSION_PROMPT_TIMEOUT_SECONDS}s`,
		"",
		"Configuration",
		`  Project config: ${paths.projectPath}`,
		`  Global config:  ${paths.globalPath}`,
		"",
		"Filesystem (bash + read/write/edit/ls/grep/find tools)",
		...
		"",
		"Rules",
		"  Filesystem-only sandboxing: bash network access is unrestricted.",
		"  Reads are blocked unless the path is in allowRead or allowWrite.",
		"  allowWrite also grants read access to the same path.",
		"  denyRead is overridden when a permission prompt is granted for a path.",
